...
def generate_vpc_id(
    ...
    # Generate a predictable VPC ID based on the CIDR block
    hash_id = hashlib.md5(resource_identifier.name.encode()).hexdigest()[:17]
    return f"vpc-{hash_id}"
...
def generate_subnet_id(
    ...
    # Generate a predictable subnet ID based on the resource identifier name
    # which contains vpc_id and cidr_block (e.g., "subnet-vpc-12345-10.0.0.0/24")
    hash_id = hashlib.md5(resource_identifier.name.encode()).hexdigest()[:17]
    return f"subnet-{hash_id}"
